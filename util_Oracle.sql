...
WHERE 
ROWNUM = 1 
AND T1.CodFlow = (SELECT T2.CodFlow FROM WFFLOW T2 WHERE DsFlowName = 'Nome Do Processo')
...
BEGIN
  ...
  FOR Fonte IN (SELECT * FROM WFGROUP_VALUE WHERE DsType = 'wsdl') LOOP
            ...
            --VERIFICA SE EXISTE O PREFIXO "http://enderecodev.com" NA URL DA FONTE DE DADOS E SALVA O RESULTADO NA VARIAVEL "CheckExists".
            BEGIN
            SELECT INSTR(DsFileName, 'http://enderecodev.com', 1)
            into CheckExists 
            FROM WFGROUP_VALUE 
            WHERE 
            ...
            IF CheckExists > 0 THEN  
                 --FAZ UM SELECT REMOVENDO O PREFIXO "http://enderecoantigo.com" do DsFileName.
                 SELECT REPLACE(DsFileName, 'http://enderecoantigo.com', 'https://endereconovo.com')
                 into NewDsFileName
                 FROM WFGROUP_VALUE
                 WHERE
                 CodGroupValue = Fonte.CodGroupValue; 
                 
                 --FAZ UM SELECT REMOVENDO O PREFIXO "http://enderecoantigo.com" do DsLocation.
                 SELECT REPLACE(DsLocation, 'http://enderecoantigo.com', 'https://endereconovo.com')
                 into NewDsLocation
                 FROM WFGROUP_VALUE
                 WHERE
                 CodGroupValue = Fonte.CodGroupValue; 
